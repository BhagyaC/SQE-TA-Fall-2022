...
public class ImportTask extends ProgressiveTask {
    ...
    public ImportTask(JsonReader reader, JsonObject root, ResourceBundle bundle, User user) {
        this.reader = reader;
        this.bundle = bundle;
        this.user = user;
        ...
        compoundEventDetectors = nonNullList(root, EmportDwr.COMPOUND_EVENT_DETECTORS);
    ...
    protected void runImpl() {
        try {
            ...
            if (scheduledEventIndex < scheduledEvents.size()) {
                importScheduledEvent(scheduledEvents.get(scheduledEventIndex++).toJsonObject());
                return;
            }

            if (compoundEventDetectorIndex < compoundEventDetectors.size()) {
                importCompoundEventDetector(compoundEventDetectors.get(compoundEventDetectorIndex++).toJsonObject());
                return;
            }
            ...
            if (maintenanceEventIndex < maintenanceEvents.size()) {
                importMaintenanceEvent(maintenanceEvents.get(maintenanceEventIndex++).toJsonObject());
                return;
            }
            ...
            while ((t = t.getCause()) != null)
                msg += ", " + I18NUtils.getMessage(bundle, "emport.causedBy") + " '" + t.getMessage() + "'";
        ...
        if (StringUtils.isEmpty(username))
            ...
            try {
                ...
                if (userResponse.getHasMessages())
                    ...
                    copyValidationMessages(userResponse, "emport.user.prefix", username);
                else {
                    // Sweet. Save it.
                ...
                response.addGenericMessage("emport.user.prefix", username, e.getMsg());
            }
    ...
    private void importDataSource(JsonObject dataSource) {
        ...
        if (StringUtils.isEmpty(xid))
            response.addGenericMessage("emport.dataSource.xid", name == null ? "(undefined)" : name);
        else {
            DataSourceVO<?> vo = dataSourceDao.getDataSource(xid);
            if (vo == null) {
                String typeStr = dataSource.getString("type");
                if (StringUtils.isEmpty(typeStr))
                    response.addGenericMessage("emport.dataSource.missingType", xid, DataSourceVO.Type.getTypeList());
                else {
                    DataSourceVO.Type type = DataSourceVO.Type.valueOfIgnoreCase(typeStr);
                    if (type == null)
                        response.addGenericMessage("emport.dataSource.invalidType", xid, typeStr,
            ...
            if (vo != null) {
                try {
                    ...
                    if (voResponse.getHasMessages())
                        // Too bad. Copy the errors into the actual response.
                        copyValidationMessages(voResponse, "emport.dataSource.prefix", xid);
                    else {
                        // Sweet. Save it.
                    ...
                    response.addGenericMessage("emport.dataSource.prefix", xid, e.getMsg());
                }
                catch (JsonException e) {
    ...
    private void importDataPoint(JsonObject dataPoint) {
        ...
        if (StringUtils.isEmpty(xid))
            response.addGenericMessage("emport.dataPoint.xid", name == null ? "(undefined)" : name);
        else {
            DataSourceVO<?> dsvo;
            ...
            if (vo != null) {
                try {
                    ...
                    if (voResponse.getHasMessages())
                        // Too bad. Copy the errors into the actual response.
                        copyValidationMessages(voResponse, "emport.dataPoint.prefix", xid);
                    else {
                        // Sweet. Save it.
                        boolean isnew = vo.isNew();
                    ...
                    response.addGenericMessage("emport.dataPoint.prefix", xid, e.getMsg());
                }
                catch (JsonException e) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                ...
                if (viewResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(viewResponse, "emport.view.prefix", xid);
                else {
                    // Sweet. Save it.
                ...
                response.addGenericMessage("emport.view.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importUserPermissions(JsonObject userJson) {
        ...
        try {
            ...
            response.addGenericMessage("emport.userPermission.prefix", username, e.getMsg());
        }
        catch (JsonException e) {
    ...
    private void importPointHierarchy(JsonArray pointHierarchyJson) {
        ...
        try {
            ...
            response.addGenericMessage("emport.pointHierarchy.prefix", I18NUtils.getMessage(bundle, "emport.saved"));
        }
        catch (LocalizableJsonException e) {
            response.addGenericMessage("emport.pointHierarchy.prefix", e.getMsg());
        }
        catch (JsonException e) {
    ...
    private void importPointLink(JsonObject pointLink) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                ...
                if (voResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(voResponse, "emport.pointLink.prefix", xid);
                else {
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                ...
                response.addGenericMessage("emport.pointLink.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importScheduledEvent(JsonObject scheduledEvent) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                ...
                if (voResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(voResponse, "emport.scheduledEvent.prefix", xid);
                else {
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                    Common.ctx.getRuntimeManager().saveScheduledEvent(vo);
                    addSuccessMessage(isnew, "emport.scheduledEvent.prefix", xid);
                }
            }
            catch (LocalizableJsonException e) {
                response.addGenericMessage("emport.scheduledEvent.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importCompoundEventDetector(JsonObject compoundEventDetector) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            CompoundEventDetectorVO vo = compoundEventDetectorDao.getCompoundEventDetector(xid);
            if (vo == null) {
            ...
            try {
                ...
                if (voResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(voResponse, "emport.compoundEvent.prefix", xid);
                else {
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                    Common.ctx.getRuntimeManager().saveCompoundEventDetector(vo);
                    addSuccessMessage(isnew, "emport.compoundEvent.prefix", xid);
                }
            }
            catch (LocalizableJsonException e) {
                response.addGenericMessage("emport.compoundEvent.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importMailingList(JsonObject mailingList) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                ...
                if (voResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(voResponse, "emport.mailingList.prefix", xid);
                else {
                    // Sweet. Save it.
                ...
                response.addGenericMessage("emport.mailingList.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importPublisher(JsonObject publisher) {
        ...
        if (StringUtils.isEmpty(xid))
            response.addGenericMessage("emport.publisher.xid", name == null ? "(undefined)" : name);
        else {
            PublisherVO<?> vo = publisherDao.getPublisher(xid);
            if (vo == null) {
                String typeStr = publisher.getString("type");
                if (StringUtils.isEmpty(typeStr))
                    response.addGenericMessage("emport.publisher.missingType", xid, PublisherVO.Type.getTypeList());
                else {
                    PublisherVO.Type type = PublisherVO.Type.valueOfIgnoreCase(typeStr);
                    if (type == null)
                        response.addGenericMessage("emport.publisher.invalidType", xid, typeStr,
            ...
            if (vo != null) {
                try {
                    ...
                    if (voResponse.getHasMessages())
                        // Too bad. Copy the errors into the actual response.
                        copyValidationMessages(voResponse, "emport.publisher.prefix", xid);
                    else {
                        // Sweet. Save it.
                        boolean isnew = vo.isNew();
                    ...
                    response.addGenericMessage("emport.publisher.prefix", xid, e.getMsg());
                }
                catch (JsonException e) {
    ...
    private void importEventHandler(JsonObject eventHandler) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                // Find the event type.
                EventType eventType = reader.readObject(eventHandler.getJsonObject("eventType"), EventType.class);
                ...
                if (voResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(voResponse, "emport.eventHandler.prefix", xid);
                else {
                    // Sweet.
                ...
                response.addGenericMessage("emport.eventHandler.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importWatchList(JsonObject watchListJson) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                ...
                response.addGenericMessage("emport.watchList.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void importMaintenanceEvent(JsonObject maintenanceEvent) {
        ...
        if (StringUtils.isEmpty(xid))
            ...
            try {
                ...
                if (voResponse.getHasMessages())
                    // Too bad. Copy the errors into the actual response.
                    copyValidationMessages(voResponse, "emport.maintenanceEvent.prefix", xid);
                else {
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                    Common.ctx.getRuntimeManager().saveMaintenanceEvent(vo);
                    addSuccessMessage(isnew, "emport.maintenanceEvent.prefix", xid);
                }
            }
            catch (LocalizableJsonException e) {
                response.addGenericMessage("emport.maintenanceEvent.prefix", xid, e.getMsg());
            }
            catch (JsonException e) {
    ...
    private void copyValidationMessages(DwrResponseI18n voResponse, String key, String desc) {
    ...
    private void addSuccessMessage(boolean isnew, String key, String desc) {
        if (isnew)
            response.addGenericMessage(key, desc, I18NUtils.getMessage(bundle, "emport.added"));
        else
            response.addGenericMessage(key, desc, I18NUtils.getMessage(bundle, "emport.saved"));
    }
        ...
        while ((t = t.getCause()) != null) {
            if (t instanceof LocalizableJsonException)
                msg += ", " + I18NUtils.getMessage(bundle, "emport.causedBy") + " '"
                        + ((LocalizableJsonException) t).getMsg().getLocalizedMessage(bundle) + "'";
            else
                msg += ", " + I18NUtils.getMessage(bundle, "emport.causedBy") + " '" + t.getMessage() + "'";
        }
