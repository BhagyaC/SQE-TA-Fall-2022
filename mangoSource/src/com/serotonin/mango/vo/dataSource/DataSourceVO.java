...
abstract public class DataSourceVO<T extends DataSourceVO<?>> implements Serializable, Cloneable, JsonSerializable,
    ...
    public enum Type {
        ...
        // insert into systemSettings (settingName, settingValue) values ('OPC.display', 'Y')
        OPC(25, "dsEdit.opc", false) {
            @Override
            public DataSourceVO<?> createDataSourceVO() {
                return new OPCDataSourceVO();
            }
        },
        OPEN_V_4_J(19, "dsEdit.openv4j", false) {
            @Override
            public DataSourceVO<?> createDataSourceVO() {
    ...
    protected EventTypeVO createEventType(int eventId, LocalizableMessage message, int duplicateHandling,
        ...
        return new EventTypeVO(EventType.EventSources.DATA_SOURCE, getId(), eventId, message, getAlarmLevel(eventId,
                defaultAlarmLevel), duplicateHandling);
    }
    ...
    public final void addPropertyChanges(List<LocalizableMessage> list, T from) {
        AuditEventType.maybeAddPropertyChangeMessage(list, "dsEdit.head.name", ((DataSourceVO)from).name, name);
        AuditEventType.maybeAddPropertyChangeMessage(list, "common.xid", ((DataSourceVO)from).xid, xid);
        AuditEventType.maybeAddPropertyChangeMessage(list, "common.enabled", ((DataSourceVO)from).enabled, enabled);
            ...
            if (eventCodes != null && eventCodes.size() > 0) {
                for (String code : alarmCodeLevels.getProperties().keySet()) {
                    ...
                    if (!eventCodes.isValidId(eventId))
                        throw new LocalizableJsonException("emport.error.eventCode", code, eventCodes.getCodeList());

                    String text = alarmCodeLevels.getString(code);
                    int level = AlarmLevels.CODES.getId(text);
                    if (!AlarmLevels.CODES.isValidId(level))
                        throw new LocalizableJsonException("emport.error.alarmLevel", text, code,
    ...
    protected void serializeUpdatePeriodType(Map<String, Object> map, int updatePeriodType) {
        map.put("updatePeriodType", Common.TIME_PERIOD_CODES.getCode(updatePeriodType));
    }

    protected Integer deserializeUpdatePeriodType(JsonObject json) throws JsonException {
        ...
        if (value == -1)
            throw new LocalizableJsonException("emport.error.invalid", "updatePeriodType", text,
